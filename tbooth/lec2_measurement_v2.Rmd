...
- **Categorical:** Variables with a *discrete* number of response options.
	...
	- **Binary** variables are a special case with only 2 categories
- Example: Hair colour, Likert Scale items, Degree or Not?

```{r echo = FALSE}
examples <- tibble(
  ID = c(paste("ID", seq(101:105), sep = "")),
...
2. Reading: Linked at the top of lab 2.
    - Additional reading on final slide. 
3. Quiz 2A (On course guide) and 2B (to show you question types)
    - Both live now.
